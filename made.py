...
class MADE(nn.Module):
    def __init__(self, nin, hidden_sizes, nout, orderings):
        """
    ...
    def update_masks(self):
        if self.m and len(self.orderings) == 1: return # only a single seed, skip for efficiency
        L = len(self.hidden_sizes)

        # fetch the next seed and construct a random stream
        rng = np.random.RandomState(self.seed)
        self.seed = (self.seed + 1) % len(self.orderings)
